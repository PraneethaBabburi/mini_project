...
filename = "data/Data_Combined_Manually.xlsx"
filename_car = "data/Data_Set_Car.xlsx"
metrics_file = "data/Metrics_File.txt"
filename_car_processed = "data/Data_Set_Car_Processed.xlsx"
filename_MOC_car = "data/Predictions_Multi_Class_Car.xlsx"
metrics_file_car = "data/Metrics_File_Car.txt"
filename_analysis = "data/Final_Results/Predictions_Multi_Class_Car_PR_ROC.xlsx"
filename_analysis_DNN = "data/Final_Results/Predictions_PR_ROC.xlsx"
filename_analysis_Keras = "data/Final_Results/Predictions_PR_ROC_Keras.xlsx"
filename_BC = "data/Predictions_Binary_Class.xlsx"

features = ['Flowrate', 'Speed', 'Phasetime']
data_car_columns = ['Vehicle no', 'Time 1', 'Time 2 (T1+30 sec)', 'Speed 1', 'Speed 2', 'Distance 1', 'Distance 2',
                    'Camera', 'Lidar', 'Radar']
target_car = ['C', 'L', 'R']
target = ['Two_Class']
...
def read_data_car(filename_car, data_car_columns, sheet_name='data_car'):
    ...
    for x in list(df.index):
        ...
        frame.append((df.iloc[x, :]['Speed 1'] - df.iloc[x, :]['Speed 2']) * (
                    df.iloc[x, :]['Distance 1'] - df.iloc[x, :]['Distance 2']))
...
def keras_MOClassifier_car(train_data, test_data, target_columns, feature_columns):
    ...
    input_dict = {
        ...
        # should be sigmoid at the end according to https://www.depends-on-the-definition.com/guide-to-multi-label-classification-with-neural-networks/
        'k_fold': 0,
        'loss': 'binary_crossentropy',
        # treat each output as a binary classifier - thus ensure to trigger the output indipendantly from others
        'optimizer': 'adam',
...
def Keras_classifier_all_data():
    ...
    Model_Eval.ROC_Curve_Generator(df, len(target),
                                   1)  # Ofset to the real probability not the class level, see the df and the generated excel file

    Model_Eval.metrics_file_writer(metrics_file_car, df, len(target), 2)

    Model_Eval.PR_Curve_Generator(df, len(target), 1, df_combined, list(df_combined.columns).index(
        'Two_Class'))  # adjust to 3 when phasetime will be added as feature set
...
def Keras_multiclass_cassifier_car():
    ...
    Model_Eval.ROC_Curve_Generator(df, len(target_car),
    ...
    Model_Eval.PR_Curve_Generator(df, len(target_car), 3, data, list(data.columns).index(
...
def final_result_analysis():
    ...
    Model_Eval.ROC_Curve_Generator_Subplot(df_final, len(target_car),
                                           3)  # Ofset to the real probability not the class level, see the df and the generated excel file

    Model_Eval.metrics_file_writer(metrics_file_car, df_final, len(target_car), 6)

    Model_Eval.PR_Curve_Generator_Subplot(df_final, len(target_car), 3, data, list(data.columns).index(
        'C'))  # adjust to 3 when phasetime will be added as feature set
...
def final_result_analysis_DNN_Keras(Model):
    ...
    Model_Eval.ROC_Curve_Generator(df_final, len(target),
                                   1)  # Ofset to the real probability not the class level, see the df and the generated excel file

    Model_Eval.metrics_file_writer(metrics_file_car, df_final, len(target), 2)
    Model_Eval.PR_Curve_Generator(df_final, len(target), 1, df_combined, list(df_combined.columns).index(
        'Two_Class'))  # adjust to 3 when phasetime will be added as feature set
    Model_Eval.figure_generator_single_output(df_final, len(target), 1, df_combined,
                                              list(df_combined.columns).index('Two_Class'))
